...
class citrixbrute():
    ...
    def appDetect(self, config, c, cookie1, cookies):
       ...
       resp1 = c.get(config["protocol"] + '://'+config["HOST"]+':' + config["port"] +'/Citrix/XenAppCAGProd23/', cookies=cookies, allow_redirects=True, verify=False)
       ...
       resp = c.get(config["protocol"] + '://'+ config["HOST"] + ':' + config["port"] + '/Citrix/XenAppCAGProd23/auth/login.aspx', cookies=cookies2, allow_redirects=False, verify=False)
       resp2 = c.get(config["protocol"] + '://'+ config["HOST"] + ':' + config["port"] + '/Citrix/XenAppCAGProd23/auth/agesso.aspx', cookies=cookies2, allow_redirects=False, verify=False)
