...
class MobileIron():
    def connectTest(self, config, payload):
        with session() as c:
            resp1 = c.get(config["protocol"] + '://' + config["HOST"] + ':' + config["port"] + '/employee/login.jsp', verify=True)
            ...
            c.cookies.clear()
            cpost = c.post(config["protocol"] + '://' + config["HOST"] + ':' + config["port"] + '/employee/j_spring_security_check', cookies=cookies, data=payload, allow_redirects=True, verify=True)
